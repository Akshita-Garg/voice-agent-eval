# Fixed-Audio Comparison — R1, R2 and R3
...
R1 versus R2 isolates LiveKit's maximum delay. R2 versus R3 changes the Pulse
...
| Measure across six calls/configuration | R1 | R2 | R3 |
|---|---:|---:|---:|
...
R3 produced the fewest transcript segments and committed turns. That grouping
...
| Metric, mean of two repetitions | R1 | R2 | R3 |
|---|---:|---:|---:|
| LiveKit end-of-utterance delay | 0.823 s | 0.844 s | **2.660 s** |
| Transcription delay within EOU | — | 0.824 s | **2.377 s** |

The R3 clean-request EOU delay was about 3.2 times R2's in this small sample.
...
| Fixture | Measured internal pause | R1 turns | R2 turns | R3 turns | R3 timing |
|---|---:|---:|---:|---:|---|
| Hesitation | 1.204 s | 1 / 2 | 1 / 2 | **1 / 1** | EOU 3.009 / 3.003 s |
| Details | 0.713 s, 1.519 s | 3 / 3 | 2 / 2 | **2 / 2** | Each run still split once |

R3 consistently merged the hesitation fixture, but the user then waited about
...
One R3 clean repetition transcribed “appointment” as “in a point.” The other
...
Select **R2**: Pulse native endpointing on, Pulse EOU timeout 100 ms, and
LiveKit semantic endpointing at 0.40–3.00 seconds.

R2 is not perfect—it retained internal fragmentation and can still wait the
full three seconds when the semantic detector is uncertain. It is the best
tested balance because it reduced R1's fragmentation while keeping clean-turn
EOU around 0.8 seconds. Reject R3 for the MVP: one fewer committed turn across
